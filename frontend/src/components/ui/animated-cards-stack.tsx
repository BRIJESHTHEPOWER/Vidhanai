...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-4 rounded-[24px] border border-[rgba(255,255,255,0.08)] bg-[#0A0A0A] p-8 shadow-[0_20px_60px_rgba(0,0,0,0.8)] backdrop-blur-[20px]",
      light:
...
export const CardsContainer: React.FC<React.HTMLAttributes<HTMLDivElement>> = ({
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative transform-style-3d", className)}
      style={{ perspective: "1200px", transformStyle: "preserve-3d", ...props.style }}
      {...props}
    >
      {children}
    </div>
...
>(
  (
    ...
    const baseRotate = index === 0 ? 0 : -(8 + (index - 1) * 6)
    const baseX = index === 0 ? 0 : -(20 + (index - 1) * 20)
    const baseY = index === 0 ? 0 : (10 + (index - 1) * 10)

    const rotate = useTransform(scrollYProgress, rotateRange, [baseRotate, 0])
    const x = useTransform(scrollYProgress, rotateRange, [baseX, 0])
    const yPx = useTransform(scrollYProgress, rotateRange, [baseY, 0])
    const yPercent = useTransform(scrollYProgress, range, [0, -180])

    const transform = useMotionTemplate`translateZ(${index * -20}px) translateX(${x}px) translateY(calc(${yPx}px + ${yPercent}%)) rotate(${rotate}deg)`
    ...
    return (
      <motion.div
        ...
        animate={{ y: 0, opacity: 1, transition: { type: "spring", stiffness: 100, damping: 20, delay: index * 0.1 } }}
